...
import { customerPatchSchema } from "@/lib/validation/schemas";
...
export async function PATCH(req: Request, ctx: Ctx) {
  try {
    const session = await requireUser(req);
    const { id } = await ctx.params;
    const body = customerPatchSchema.parse(await parseJson(req));
    ...
    if (body.loyaltyPoints !== undefined) {
      fields.push(`loyalty_points = $${i++}`);
      vals.push(body.loyaltyPoints);
    }
    ...
    if (fields.length === 0) {
      ...
      return jsonOk({ customer: cur.rows[0] });
    }
    fields.push(`updated_at = now()`);
    vals.push(id, businessId);
    const q = `UPDATE customers SET ${fields.join(", ")} WHERE id = $${i++} AND business_id = $${i} AND deleted_at IS NULL RETURNING *`;
